...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        # Support both square and non-square grids
        if isinstance(pt_seq_len, (tuple, list)):
            # Non-square: pt_seq_len = (height, width)
            pt_h, pt_w = pt_seq_len
            if ft_seq_len is None:
                ft_h, ft_w = pt_h, pt_w
            elif isinstance(ft_seq_len, (tuple, list)):
            ...
            # Square grid (original implementation)
            if ft_seq_len is None: ft_seq_len = pt_seq_len
            t = torch.arange(ft_seq_len) / ft_seq_len * pt_seq_len
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    """
    grid_size: int of the grid height and width, or tuple (height, width) for non-square grids
    return:
    ...
    if isinstance(grid_size, int):
        ...
        # Support non-square grids: grid_size = (height, width)
